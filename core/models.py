...
class SiteSettings(models.Model):
    """Site ayarları - tüm metinler buradan yönetilir"""
...
class GaleriResim(models.Model):
    ...
    image = models.ImageField(
        upload_to='galeri/%Y/%m/%d/',
        verbose_name="Resim",
        validators=[FileExtensionValidator(['jpg', 'jpeg', 'png', 'webp', 'gif', 'svg'])]
    )
    ...
    def image_preview(self):
        if self.image:
            return f'<img src="{self.image.url}" width="100" height="100" style="object-fit:cover;border-radius:4px;" />'
        return '-'
...
class Hizmet(models.Model):
    ...
    image = models.ImageField(
        upload_to='hizmetler/kapak/',
        blank=True,
        null=True,
        verbose_name="Обложка",
        validators=[FileExtensionValidator(['jpg', 'jpeg', 'png', 'webp', 'gif'])]
    )
    gallery_images = models.JSONField(default=list, blank=True, verbose_name="Галерея (JSON - устаревший)")
